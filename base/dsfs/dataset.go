...
func CreateDataset(ctx context.Context, source, destination cafs.Filestore, ds, prev *dataset.Dataset, pk crypto.PrivKey, sw SaveSwitches) (string, error) {
	...
	path, err := WriteDataset(ctx, dsLk, destination, ds, sw.Pin)
	if err != nil {
		log.Debug(err.Error())
...
func WriteDataset(ctx context.Context, dsLk *sync.Mutex, destination cafs.Filestore, ds *dataset.Dataset, pin bool) (string, error) {
